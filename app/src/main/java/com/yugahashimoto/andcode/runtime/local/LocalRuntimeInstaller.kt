...
class LocalRuntimeInstaller(
    ...
    /**
     * Registers the guest-device MCP server with every agent (OpenCode, Claude Code, Antigravity)
     * so they all expose device_* tools for phone automation.
     */
    private fun provisionDeviceMcp(rootfs: File) {
        mergeJsonConfig(File(rootfs, "root/.config/opencode/opencode.json")) { root ->
            val mcp = root.optJSONObject("mcp") ?: JSONObject()
            mcp.put(DEVICE_MCP_NAME, deviceMcpEntry("opencode"))
            root.put("mcp", mcp)
        }
        mergeJsonConfig(File(rootfs, "root/.claude.json")) { root ->
            val servers = root.optJSONObject("mcpServers") ?: JSONObject()
            servers.put(DEVICE_MCP_NAME, deviceMcpEntry("claude"))
            root.put("mcpServers", servers)
        }
        mergeJsonConfig(File(rootfs, "root/.gemini/config/mcp_config.json")) { root ->
            val servers = root.optJSONObject("mcpServers") ?: JSONObject()
            servers.put(DEVICE_MCP_NAME, deviceMcpEntry("antigravity"))
            root.put("mcpServers", servers)
        }
    }

    private fun deviceMcpEntry(agent: String): JSONObject =
        when (agent) {
            "claude" ->
                JSONObject()
                    .put("type", "stdio")
                    .put("command", DEVICE_MCP_BIN)
                    .put("args", JSONArray())
            "antigravity" -> JSONObject().put("command", DEVICE_MCP_BIN)
            else ->
                JSONObject()
                    .put("type", "local")
                    .put("command", JSONArray(listOf(DEVICE_MCP_BIN)))
                    .put("enabled", true)
                    .put("timeout", DEVICE_MCP_TIMEOUT_MILLIS)
        }
    ...
    private fun installAndroidHelperScripts(rootfs: File) {
        ...
        listOf(
            ...
            "andcode-device-mcp.py" to "andcode-device-mcp.py",
        ).forEach {
    ...
    internal companion object {
        ...
        private const val DEVICE_MCP_NAME = "and-code-device"
        private const val DEVICE_MCP_BIN = "/usr/local/bin/andcode-device-mcp.py"
        private const val DEVICE_MCP_TIMEOUT_MILLIS = 45000
