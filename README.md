...
- **Autonomous Phone Agent (Siri-like capabilities)** — Full device automation through `and-code-device` MCP tools:
  - **App launcher** — Open any app by voice or text in Arabic or English ("افتح واتس", "open YouTube")
  - **Voice calls & Contacts** — Resolve contacts and place phone calls or dial numbers ("اتصل بمحمد", "call Dad")
  - **WhatsApp automation** — Pre-fill and auto-send WhatsApp messages with Accessibility Service assistance ("ارسل رسالة واتساب لعمر")
  - **Device media & file search** — Fast MediaStore and filesystem indexing for videos, photos, and files ("ابحث عن فيديو البحر")
  - **Exact timers & delayed scheduling** — Set timers or schedule actions in the future ("ارسل رسالة بعد 10 دقائق")
  - **Hardware & telemetry** — Toggle flashlight, check battery percentage, storage, and device status
  - **Native Accessibility Service** — Tap buttons, type, and inspect screens without requiring ADB or a PC
